...
공개 메력서의 작성자만 `동행 확인 링크 만들기`를 눌러 7일 안에 한 번 사용할 수 있는 링크를 발급할 수 있습니다. 링크는 실제 함께한 파티원에게만 전달해야 하며, 받은 사람은 **본인이 관리하는 공개 메력서**의 주소와 같은 브라우저의 편집 권한으로 긍정 태그 1~3개를 남깁니다.

- 태그: 약속 시간 준수, 공략 준비, 원활한 소통, 재도전 적극성, 공정한 분배
- 링크 원문은 URL의 `#invite` fragment로만 전달되고 DB에는 hash만 저장됩니다.
- 숫자, 별점, 평균, 순위, 부정 태그, 자유 댓글, 익명 제출은 제공하지 않습니다.
- 자기 평가·같은 캐릭터의 중복 기록·만료/재사용 링크는 차단됩니다.
- 동행 기록은 해당 공개 버전의 검증 페이지에만 표시되며 1080×1350 PNG·QR 공유 이미지·이력서 글 복사에는 포함되지 않습니다.

## 나의 이력서: 보스별 저장 목록

상단의 `나의 이력서`에서 같은 브라우저로 만든 메력서를 보스별로 다시 열 수 있습니다. 각 카드에서 공개 보기, 수정, `새 메력서로 저장`을 선택할 수 있으므로 같은 캐릭터의 보스별·일자별 기록을 기존 URL과 별도로 보관할 수 있습니다.
