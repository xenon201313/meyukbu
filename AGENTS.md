...
- 합격/불합격 판정, 직업별 단일 종합 점수, 블랙리스트는 만들지 않는다. 단, 사용자가 명시 승인한 `메붕이 온도`는 실제 파티 경험을 위한 한정된 익명 설문 예외로, 캐릭터별 36.5℃ 기준 누적값만 표시하며 합격 판단·매칭 순위·직업 비교에는 절대 사용하지 않는다.
- 직업별 편향을 줄이기 위해 역할별로 정보의 표시 우선순위만 다르게 한다. 메붕이 온도는 직업·역할·합격 여부를 평가하거나 이용자를 비교·정렬하는 값으로 사용하지 않는다.
