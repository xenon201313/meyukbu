...
test("mock 검색부터 게시, 검증, PNG 및 버전 갱신까지 동작한다", async ({ page }) => {
  ...
  const imageUrl = `${publicPath}/image?v=1&layout=8`;
...
test("최대 6개 보스 묶음도 1080×1350 PNG와 하단 검증 영역을 유지한다", async ({ page }) => {
  ...
  expect(imageUrl).toMatch(/^\/r\/m-[a-z0-9_-]+\/image\?v=1&layout=8$/);
  if (!imageUrl) {
...
test("메붕이 온도 설문은 익명 3문항으로 한 번만 제출되고 같은 캐릭터의 새 메력서에도 남는다", async ({
  ...
  expect(originalImageUrl).toMatch(/^\/r\/m-[a-z0-9_-]+\/image\?v=1&layout=8$/);
  if (!originalImageUrl) {
    throw new Error("Expected the published resume to have a share image URL.");
  }
  const originalImageResponse = await page.request.get(originalImageUrl);
  expect(originalImageResponse.ok()).toBeTruthy();
  expect(originalImageResponse.headers()["cache-control"]).toContain("no-store");
  ...
  await expect(publicGauge).toContainText("41.5℃");
  ...
  const updatedImageResponse = await page.request.get(originalImageUrl);
  expect(updatedImageResponse.ok()).toBeTruthy();
  expect(updatedImageResponse.headers()["cache-control"]).toContain("no-store");
  const updatedImageBody = await updatedImageResponse.body();
  expect(Buffer.compare(updatedImageBody, originalImageBody)).not.toBe(0);
  ...
  await expect(page.getByTestId("mesoongi-temperature-gauge")).toContainText("41.5℃");
