...
- `TemperatureFeedback`: resumeId, resumeVersionId, invitationId, reviewer resume/OCID/공개 신원, 긍정 태그, published/withdrawn 상태
...
`메숭이 체온`은 별점이나 단일 온도 수치가 아닌 기명 동행 기록이다. 초대장은 작성자 edit token으로만 만들며 7일 뒤 만료되고 한 번만 소비된다. 동행 기록을 제출하는 사람은 자신의 공개 메력서 slug와 그 edit token cookie를 제시해야 한다. 서비스는 같은 OCID의 자기 평가를 거부하고, 저장소는 `[resumeId, reviewerOcid]`와 `invitationId`를 각각 unique로 보장해 같은 캐릭터의 중복 평가와 초대장 재사용을 막는다. 공개 조회는 대상 version과 평가자 메력서가 모두 현재 public일 때만 태그와 평가자 링크를 반환한다.
...
7. 동행 확인 링크 발급은 대상 작성자 edit token이 필요하며, 동행 기록 제출은 별도 평가자 메력서의 edit token을 검증한다. token·자유 댓글은 로그에 남기지 않는다.
