...
interface ResumeDraft {
  ...
  // Legacy primary aliases retained only to render immutable scalar versions.
  targetBoss: string;
...
지원 분야에는 작성 내용인 보스 묶음(1~6개), 보스별 배율, 희망 인원이 함께 저장된다. 새 version은 stable `bossId`에서 보스명·주기를 server-side로 정규화하고 첫 target을 legacy scalar alias로 동기화한다. 보스 카탈로그는 `maxPartySize`를 보유하며 서버 Zod 검증과 편집기 옵션이 묶음의 가장 엄격한 값으로 제한한다. 스우는 최대 2인, 최초의 대적자·림보·발드릭스·찬란한 흉성·유피테르는 최대 3인이고, 나머지는 최대 6인이다. `availabilityMode`가 없는 과거 버전은 `SCHEDULED`로 해석하며 `partySize`가 없는 과거 버전은 `미입력`으로 표시한다.
...
- `PartyPost`: ownerResumeId, pinned ResumeVersion, 모집/파티 찾기 유형, 공개 상태, 만료 시각
- `PartyPostTarget`: PartyPost에 복사한 보스명·주기·작성 배율·입장 인원 제한·순서
- `PartyApplication`: applicant ResumeVersion, 상태(대기/수락/거절), 소유자 전용 짧은 메시지. 연락처·token·OCID는 공개 DTO에 포함하지 않는다.

공개 이미지는 immutable ResumeVersion snapshot을 기반으로 생성하되, 메붕이 온도는 Character 단위의 live aggregate로 별도 결합한다. 이 값이 포함된 PNG는 immutable CDN cache를 쓰지 않고 매 요청 최신 집계값으로 렌더링한다.
...
- 1080×1350 PNG
- 서버에서 동일 version snapshot과 현재 메붕이 온도 집계를 렌더링
- 외부 이미지 fetch 실패 시 initials/placeholder
- QR은 canonical verification URL을 가리킨다.
- 한국어 폰트는 라이선스가 명확한 패키지/자체 호스팅 방식만 사용한다.
- 메붕이 온도를 포함하는 이미지 route는 `no-store` cache header를 사용해 설문 반영 뒤 오래된 PNG를 제공하지 않는다.
- 이미지에 API key, edit token, 비공개 contact를 포함하지 않는다.

## 7-1. 글 양식 복사

- 서버 컴포넌트가 현재 `PublicResumeView`, 메붕이 온도 집계, canonical URL에서 고정 순서의 plain text를 만든다.
...
7. 메붕이 온도 설문 링크 발급은 대상 작성자 edit token이 필요하며, 익명 제출은 1회용 token hash만 검증한다. token·설문자 식별자·개별 응답은 로그에 남기지 않는다.
