...
5. 주간/월간 선택 카드는 작성 내용의 지원 분야를 돕는 UI다. 보스 일러스트는 사용자가 사용을 허가한 `maple-trackers.com` 원본을 `/api/boss-art/[key]` 어댑터로 요청한다. 이 저장소에는 해당 이미지를 복제하지 않으며, source가 꺼져 있거나 offline 테스트 중이면 중립적인 fallback을 사용한다.
...
- 보스 구분과 보스명·난이도는 `작성 내용` 배지를 유지하고, 장비·전투력은 `API 조회` 배지를 유지한다.
...
- 외부 보스 아트는 사용자가 제공한 원본 사이트에 한정한다. 보스명·난이도·주기는 작성 내용이며, 이 시각 자산이 NEXON의 공식 제휴·인증을 뜻하지 않는다.
