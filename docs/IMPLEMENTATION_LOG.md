...
- 메숭이 체온을 공개 검증 페이지의 보조 패널에만 두지 않고, 작성 중인 문서형 이력서 미리보기와 실제 1080×1350 공유 PNG의 핵심 항목으로 표시했다. 두 화면 모두 현재 온도, 0~100℃ 게이지, 익명 설문 응답 수만 보여 주며, 응답자·개별 답변·문항별 점수는 공개하지 않는다.
