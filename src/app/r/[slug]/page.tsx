  ...
  return (
    <main className="min-h-screen pb-14">
      <header className="mx-auto flex max-w-5xl items-center justify-between px-5 py-5 sm:px-8">
        ...
        <Link href="/create" className="text-sm font-semibold text-stone-600 hover:text-stone-950">
          메력서 만들기
        </Link>
      </header>
      ...
      <div className="mx-auto grid max-w-5xl gap-7 px-5 sm:px-8 lg:grid-cols-[minmax(0,1fr)_280px]">
        <section>
          {!resume.isLatestVersion ? (
            <div
              role="status"
              className="mb-4 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-sm leading-6 text-amber-950"
            >
          ...
          {resume.freshness === "expired" ? (
            <div
              role="status"
              className="mb-4 rounded-2xl border border-rose-300 bg-rose-50 p-4 text-sm leading-6 text-rose-950"
            >
        ...
        <aside className="space-y-4 lg:sticky lg:top-6 lg:h-fit">
          <section className="rounded-2xl border border-stone-300 bg-white p-4">
            ...
            <dl className="mt-3 space-y-3 text-sm">
              ...
              <div>
                <dt className="text-stone-500">source date</dt>
                <dd className="mt-1 font-medium">{resume.version.snapshot.sourceDate ?? "조회 불가"}</dd>
              </div>
              <div>
                <dt className="text-stone-500">버전 / content hash</dt>
            ...
            <a
              href={imageUrl}
              download
              className="mt-4 block rounded-xl bg-stone-950 px-4 py-3 text-center text-sm font-bold text-white hover:bg-stone-700"
            >
          ...
          <section className="rounded-2xl border border-stone-300 bg-white p-4">
            <h2 className="text-sm font-bold">출처 범례</h2>
          ...
          {resume.guildObservations.length ? (
            <section className="rounded-2xl border border-stone-300 bg-white p-4">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-sm font-bold">길드 관측</h2>
                <ProvenanceBadge provenance="SERVICE_OBSERVED" />
              </div>
              <ul className="mt-3 space-y-3 text-xs leading-5 text-stone-700">
