    ...
    <section aria-labelledby="combat-stats-heading" className="mt-7 space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2 id="combat-stats-heading" className="mt-1 text-2xl font-black tracking-tight text-[#202a36]">
            전투력과 최종 능력치
          </h2>
        </div>
      ...
      <section className="rounded-xl border border-[#d7b98a] bg-[#fbf2e3] p-5 text-[#202a36]">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <p className="text-xs font-bold tracking-[0.15em] text-[#7c2f2c]">
              {usesPeak ? "최고 전투력 (서비스 관측)" : "인게임 전투력"}
            </p>
            <p className="mt-2 text-3xl font-black tracking-tight text-[#202a36]">
              {shownValue === null ? "조회 불가" : formatNumericDisplay(shownValue)}
            </p>
          </div>
          <ProvenanceBadge provenance={usesPeak ? "SERVICE_OBSERVED" : "NEXON_API"} />
        </div>
        <p className="mt-3 text-xs leading-5 text-[#5e4030]">
        ...
        <details className="mt-4 border-t border-[#d7b98a] pt-3">
            ...
            <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 text-xs sm:grid-cols-3">
              {profile.stats.map((stat) => (
                <div key={`${stat.label}:${stat.value}`} className="border-b border-[#eadfce] pb-1">
                  <dt className="text-[#687380]">{stat.label}</dt>
                  <dd className="mt-0.5 font-semibold text-[#202a36]">{formatNumericDisplay(stat.value)}</dd>
                </div>
              ))}
            </dl>
          ) : (
            <p className="mt-2 text-xs text-[#687380]">능력치 API 조회 결과가 없습니다.</p>
          )}
        </details>
      </section>
    </section>
